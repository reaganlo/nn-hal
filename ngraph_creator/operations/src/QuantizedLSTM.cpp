...
std::shared_ptr<ngraph::Node> QuantizedLSTM::createNode() {
    ...
    mNgraphNodes->setOutputAtOperandIndex(outputIndex, QLstmOutputs[0]);

    const auto op = sModelInfo->getOperand(outputIndex);
    if (op.lifetime == V1_3::OperandLifeTime::SUBGRAPH_OUTPUT) {
        addResultNode(outputIndex, QLstmOutputs[0]);
    }

    outputIndex = sModelInfo->getOperationOutput(mNnapiOperationIndex, 1);
    mNgraphNodes->setOutputAtOperandIndex(outputIndex, QLstmOutputs[1]);

    const auto op1 = sModelInfo->getOperand(outputIndex);
    if (op1.lifetime == V1_3::OperandLifeTime::SUBGRAPH_OUTPUT) {
        addResultNode(outputIndex, QLstmOutputs[1]);
    }

    outputIndex = sModelInfo->getOperationOutput(mNnapiOperationIndex, 2);
    mNgraphNodes->setOutputAtOperandIndex(outputIndex, QLstmOutputs[2]);

    const auto op2 = sModelInfo->getOperand(outputIndex);
    if (op2.lifetime == V1_3::OperandLifeTime::SUBGRAPH_OUTPUT) {
        addResultNode(outputIndex, QLstmOutputs[2]);
    }
